-- Extro: Activities feed — baseline schema (Supabase CLI migration).
--
-- This is the first migration in the CLI-managed history: a complete
-- from-scratch build of the whole database (all tables, RLS policies,
-- the auth.users signup trigger, and the pg_cron cleanup job). It was
-- adopted from the previously hand-maintained sql/schema.sql and marked
-- already-applied on the live project via `supabase migration repair`.
-- Every schema change after this one is its own timestamped file in this
-- directory, applied with `supabase db push`. Do not edit this file.
