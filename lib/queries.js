...
export async function deleteMyAccount() {
  // Deletes the auth.users row via a security-definer function; everything
  // else cascades. Then clear the local session.
...
// Circle creation and member-adding go through security-definer functions
// that enforce the mutual-friend rule (everyone in a circle must be an
// accepted friend of everyone else) -- direct inserts are blocked by RLS.
export async function createCircle(name, memberIds) {
...
export async function deleteCircle(circleId) {
  // Members + visibility rows are removed via ON DELETE CASCADE.
...
export async function listActivitiesFull() {
  const { data, error } = await supabase
    ...
    .select(
      `id, text, category, timeframe, location, author_id, created_at,
       event_at, expire_after_hours, expires_at, tags, visible_to_all_friends,
  ...
  return data.map((a) => ({
    ...
    tags: a.tags,
    visibleToAllFriends: a.visible_to_all_friends,
...
export async function createActivity({
  ...
  const { data: activity, error } = await supabase
    ...
    .insert({
      ...
      tags,
      visible_to_all_friends: visibleToAllFriends,
    })
    .select()
    .single();
  if (error) throw error;
...
export async function listNotifications() {
  const { data, error } = await supabase
    .from("notifications")
    .select(
      `id, type, activity_id, read_at, created_at,
  ...
  return data.map((n) => ({
    ...
    activityText: n.activity?.text ?? null,
