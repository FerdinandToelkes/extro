...
export async function updateMyProfile({ name, city, bio, username }) {
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  const { error } = await supabase
    .from("profiles")
    .update({ name, city: city || null, bio: bio || null, username: normalizedUsername })
    .eq("id", user.id);
  if (error) throw error;
}

// A single current "I'm generally free" status -- setting a new one
// replaces the old. available_until reuses computeExpiresAt (defined
// below, in the Activities section) with a fixed 24-hour window and no
// exact time, which resolves to "end of the picked day" -- exactly the
// semantics wanted here too.
export async function setMyAvailability(day, timeOfDay) {
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) throw new Error("Not signed in");
  const { error } = await supabase
    ...
    .update({
      available_day: day,
      available_time_of_day: timeOfDay,
      available_until: computeExpiresAt(day, 24),
    })
    .eq("id", user.id);
  if (error) throw error;
}

export async function clearMyAvailability() {
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) throw new Error("Not signed in");
  const { error } = await supabase
    .from("profiles")
    .update({ available_day: null, available_time_of_day: null, available_until: null })
    .eq("id", user.id);
  if (error) throw error;
}
